...
const EditarUsuario = async (req: Request, res: Response) => {
  try {
    const { id, email, nombres, apellidos, direccion } = req.body;

    if (!id || !email || !nombres || !apellidos || !direccion) {
      return res.status(400).json({ message: "Faltan campos requeridos" });
    }

    const resultado = await UserService.EditarUsuario(id, email, nombres, apellidos, direccion);
